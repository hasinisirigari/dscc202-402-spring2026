...
daily_customers_df = (transactions_df
    ...
    .orderBy( desc("transaction_date") )  # Sort by which column?
)
...
transactions_with_customers_df = (transactions_df
    .join(
        customers_df,
          transactions_df['customerID']==customers_df["customerID"], # Join condition here
          "inner" # Join type: "inner", "left", etc.
    )
    .select(
        transactions_df["*"],
...
full_transaction_df = (transactions_with_customers_df
    .join(
        franchises_df,
        transactions_with_customers_df["franchiseID"] == franchises_df["franchiseID"], # Join condition on franchiseID
        "inner"
    )
    .select(
...
franchise_metrics_df = (transactions_df
    ...
    .withColumn("day_of_week", date_format(col("dateTime"), "EEEE"))
    .groupBy("franchiseID")
.agg(
        sum("totalPrice").alias("total_revenue"),
        count("*").alias("transaction_count"),
        avg("totalPrice").alias("avg_customer_spend")
    )
...
display(most_popular_payment_df)
