...
def calculate_trip_metrics(pickup_times: pd.Series, dropoff_times: pd.Series, distances: pd.Series) -> pd.DataFrame:
 ...
 # Use np.where(duration_hours > 0, distances / duration_hours, 0)
...
with mlflow.start_run(run_name= "enhanced_rf_model" ) as run: 
    ...
    mlflow.sklearn.log_model(rf_model_enh, "model", input_example=X_train_enh.head(5))
...
all_runs = mlflow.search_runs(
 ...
 # Order by metrics.rmse ASC (as a list)
)
...
try:
    ...
    model_version = mlflow.register_model(
        model_uri= best_model_uri , name=model_name # The model URI
        # The model name
    )
